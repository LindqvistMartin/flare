...
export function useUpdateActionItemStatus(itemId: string) {
  ...
  return useMutation({
    ...
    mutationFn: (vars: PatchActionItemVars) =>
      api
        .patch<ActionItem>(`/api/v1/action-items/${itemId}`, { status: vars.status ?? null })
        .then(r => r.data),
    onMutate: async (vars) => {
      if (vars.status === undefined) return { previousLists: [] }
      // Snapshot every list under ['action-items', *] so we can roll back
      // both filter views together. The dashboard summary also reads from
      // these queries on next refetch.
