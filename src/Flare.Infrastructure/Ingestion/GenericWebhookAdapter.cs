...
public sealed class GenericWebhookAdapter : IAlertIngestionAdapter
{
    ...
    public Task<IncidentCreateCommand?> ParseAsync(IngestionJob job, CancellationToken ct)
    {
        ...
        var title = root.TryGetProperty("title", out var t) ? t.GetString() ?? "Incoming alert" : "Incoming alert";

        var severityStr = root.TryGetProperty("severity", out var s) ? s.GetString() : null;
        var severity = severityStr?.ToLowerInvariant() switch
        {
            "sev1" or "critical" => IncidentSeverity.Sev1,
            "sev2" or "warning"  => IncidentSeverity.Sev2,
            "sev3" or "info"     => IncidentSeverity.Sev3,
